...
double	Bird::get_rjiggle(void) { return rjiggle; }
...
void Bird::set_rjiggle(double rijggle_) { rjiggle = rijggle_; }
...
void Bird::move(void)
{
	...
	*/
	double delta_x = cos(PI/2 - direction)* ((double)velocity);
	double delta_y = sin(PI/2 - direction)* ((double)velocity);

	pos.set_next_x(pos.get_x() + delta_x);
	pos.set_next_y(pos.get_y() + delta_y);

	double new_x = pos.get_next_x();
	double new_y = pos.get_next_y();
